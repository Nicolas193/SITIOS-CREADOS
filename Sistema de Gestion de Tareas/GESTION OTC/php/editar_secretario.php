...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['id_registro'])) die("No se indicó el ID de la tarea.");
...
// 🔹 Usuarios a eliminar
$usuarios_a_eliminar = array_diff(array_keys($vinculos_actuales), $usuarios_vinculados);
...
$stmtIns = $mysqli->prepare("INSERT INTO usuarios_vinculados (id_usuario, id_registro, id_persona_asignada) VALUES (?, ?, ?)");

// 🔹 Insertar solo usuarios nuevos
foreach ($usuarios_vinculados as $id_usuario_vinc) {
    $id_usuario_vinc = (int)$id_usuario_vinc;

    // Saltar si ya existe
    if (isset($vinculos_actuales[$id_usuario_vinc])) continue;
    ...
    // Generar id_persona_asignada único: contar registros previos + 1
    ...
    $stmt_count->execute();
    $frecuencia = ($stmt_count->get_result()->fetch_assoc()['frecuencia'] ?? 0) + 1;
    $stmt_count->close();
...
$stmtIns->close();
    ...
    header("Location: asignaciontareassecretario.php?success=1");
    exit;
}


// --- GET ---
if (!isset($_GET['id'])) die("No se indicó el ID de la tarea.");
...
<div class="wrapper-mistareas">
  <div class="container-mistareas form-container">
    ...
    <button type="button" onclick="window.location.href='asignaciontareassecretario.php'" style="background: linear-gradient(135deg, #531f5d, #531f5d);
      border: none;
      color: white;
      padding: 12px 32px;
      font-weight: 700;
    ...
    <form method="post" action="editar_secretario.php">
