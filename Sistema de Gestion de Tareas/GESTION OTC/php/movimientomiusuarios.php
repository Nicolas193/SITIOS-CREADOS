...
// Obtener tareas vinculadas al usuario logueado
$sql = '
SELECT 
    rt.fecha_solicitud,
    rt.plazo_entrega,
    ...
    rt.id_registro,
    GROUP_CONCAT(DISTINCT uv.id_persona_asignada ORDER BY uv.id_persona_asignada) AS id_persona_asignada,
...
if (!$stmt) {
    die("Error en prepare de consulta de tareas: " . $mysqli->error);
}
  ...
  <div class="wrapper-mistareas">
    <div class="container-mistareas table-container">
      ...
      <section class="task-list-section">
        ...
        <a href="exportar_csv_mismovimientos.php" class="btn-download-csv" title="Descargar CSV">
                ...
                <?php while ($fila = $resultado->fetch_assoc()): ?>
                  <tr class="clickable-row" data-id="<?= htmlspecialchars($fila['id_registro']) ?>" style="cursor: pointer;">
                    ...
                    <td style="white-space: pre-line;">
                      <?= htmlspecialchars($fila['ultimo_comentario'] ?? '') ?>
                    </td>
                    <td style="white-space: pre-line;">
                      <?= htmlspecialchars($fila['ultimo_estado'] ?? '') ?>
                    </td>
                    <td>
                      <?php
