...
<div class="wrapper-mistareas">
  <div class="container-mistareas form-container">
      ...
      <h1 class="title">Tarea Realizada</h1>
      <h2 class="subtitle">(Si tu tarea no fue registrada por ningún usuario podrás cargar tu tarea realizada (la tarea debe estar finalizada))</h2>
      ...
      <form method="post" action="guardado_mistareas.php">
  ...
  <?php endif; ?>
          ...
          <div class="form-group full-width">
            ...
            <select id="usuarios_vinculados" name="usuarios_vinculados[]" multiple="multiple" style="width: 100%;">
              <?php foreach ($usuarios as $u): ?>
                <?php if ($u['id_usuario'] == $idResponsable): ?>
                  <option value="<?= htmlspecialchars($u['id_usuario']) ?>" selected data-fixed="true">
                    <?= htmlspecialchars($u['usuario']) ?> (<?= htmlspecialchars($u['sector']) ?>) (Responsable)
                  </option>
                <?php else: ?>
                  <option value="<?= htmlspecialchars($u['id_usuario']) ?>">
                    <?= htmlspecialchars($u['usuario']) ?> (<?= htmlspecialchars($u['sector']) ?>)
  ...
  <div class="wrapper-mistareas">
    <div class="container-mistareas table-container">
      ...
      <section class="task-list-section">
        <h2 class="section-title">MIS TAREAS FINALIZADAS</h2>
        <br>
        <a href="exportar_mitareas_csv.php" class="btn-download-csv" title="Descargar CSV">
                  ...
                  <?php while ($fila = $resultado->fetch_assoc()): ?>
                    ...
                    <td><?= (new DateTime($fila['plazo_entrega']))->format('d/m/Y') ?></td>
                      ...
                      <td class="td-opciones">
                        <a href="editar_mitarea.php?id=<?= urlencode($fila['id_registro']) ?>" title="Editar">
                          <i class="fas fa-pen"></i>
                        </a>
                        <a href="borrar_mitarea.php?id=<?= urlencode($fila['id_registro']) ?>" title="Borrar" onclick="return confirm('¿Seguro que quieres borrar esta tarea?');">
