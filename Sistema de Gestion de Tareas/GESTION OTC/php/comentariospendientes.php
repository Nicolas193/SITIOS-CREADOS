...
if (isset($_SESSION['username'])) {
    $usuario = $_SESSION['username'];
      $queryResponsable = $conn->prepare("SELECT id_usuario FROM usuarios WHERE usuario = ? LIMIT 1");
      $queryResponsable->bind_param("s", $usuario);
      $queryResponsable->execute();
      $resultResp = $queryResponsable->get_result();
      if ($resultResp && $rowResponsable = $resultResp->fetch_assoc()) {
...
if ($es_responsable && !$es_encargado) {
    // Solo responsable
    $filtro_estados = "tipo = 'Evaluador'OR tipo = 'Ambos'";
...
if (!$es_responsable && !$es_encargado) {
    die("Acceso denegado. No tenés permiso para acceder a esta tarea.");
}
...
<div class="contenedor-padre">
  ...
  <a href="#" class="abrir-modal-encargados"><?= htmlspecialchars($tarea['encargados_detalle']) ?></a>
</div>
    ...
    <div class="campo">
      ...
      <span class="dias-label <?= $color_dias_restantes ?>">
        <?= $dias_restantes < 0 ? "Vencida" : ($dias_restantes . " día" . ($dias_restantes == 1 ? "" : "s")) ?>
      </span>
    </div>

    <!-- Menú para cambiar estado -->
<?php if ($filtro_estados !== null): ?>
<div class="form-cambio-estado">
  <form action="guardar_estado.php" method="POST">
    ...
    <label for="id_estado">Cambiar Estado:</label>
        <select name="id_estado" id="id_estado" required>
          ...
          <?php foreach ($estados_disponibles as $estado): ?>
            <?php if ($estado['id_estado'] != $tarea['id_estado']): ?>
...
<form action="agregar_comentariopendiente.php" method="POST" class="form-comentario">
  <input type="hidden" name="id_registro" value="<?= $id ?>">
  <textarea name="comentario" placeholder="Agregar un comentario..." required <?= $bloquear_comentario ? 'disabled' : '' ?>></textarea>
  <button type="submit" <?= $bloquear_comentario ? 'disabled style="background: #ccc; cursor: not-allowed;"' : '' ?>>Enviar</button>
</form>
