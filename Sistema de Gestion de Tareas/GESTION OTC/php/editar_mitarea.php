...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['id_registro'])) die("No se indicó el ID de la tarea para actualizar.");
    ...
    $usuarios_vinculados_nuevos = $_POST['usuarios_vinculados'] ?? [];
    ...
    if (count($usuarios_a_eliminar) > 0) {
...
$stmt = $mysqli->prepare("
    SELECT fecha_solicitud, plazo_entrega, asunto, id_tarea, id_usuario_rest
    FROM registro_de_tareas
    WHERE id_registro = ?
");
...
// Obtener id_persona_asignada (si existe)
$stmtPersona = $mysqli->prepare("SELECT id_persona_asignada FROM usuarios_vinculados WHERE id_registro = ? LIMIT 1");
$stmtPersona->bind_param("i", $id_registro);
$stmtPersona->execute();
$resultPersona = $stmtPersona->get_result();
$id_persona_asignada = $resultPersona->num_rows > 0 ? $resultPersona->fetch_assoc()['id_persona_asignada'] : '';
$stmtPersona->close();
...
$stmtVinc = $mysqli->prepare("SELECT id_usuario FROM usuarios_vinculados WHERE id_registro = ?");
...
<div class="wrapper-mistareas">
  <div class="container-mistareas form-container">
    ...
    <button type="button" onclick="window.location.href='mistareas.php'" style="background: linear-gradient(135deg, #531f5d, #531f5d);
      border: none;
      color: white;
      padding: 12px 32px;
      font-weight: 700;
    ...
    <form method="post" action="editar_mitarea.php">
      ...
      <div class="form-dates">
        <div class="form-group">
          ...
          <input type="date" name="fecha_solicitud" id="fecha_solicitud" value="<?= htmlspecialchars($tarea['fecha_solicitud']) ?>" required>
        </div>
        <div class="form-group">
          <label for="plazo_entrega">Plazo de Entrega</label>
          <input type="date" name="plazo_entrega" id="plazo_entrega" value="<?= htmlspecialchars($tarea['plazo_entrega']) ?>" required>
        </div>
      </div>

      <div class="form-group full-width">
        <label for="id_persona_asignada">ID Persona Asignada</label>
        <input type="text" id="id_persona_asignada_mostrado" value="<?= htmlspecialchars($id_persona_asignada) ?>" disabled>
      </div>

      <div class="form-group full-width">
        <label for="asunto">Asunto</label>
        <input type="text" name="asunto" id="asunto" value="<?= htmlspecialchars($tarea['asunto']) ?>" required>
      </div>
      ...
      <div class="form-group full-width">
        <label for="id_usuario_rest">Responsable:</label>
        <select name="id_usuario_rest" id="id_usuario_rest" required>
          <?php foreach ($usuarios as $u): ?>
            <option value="<?= htmlspecialchars($u['id_usuario']) ?>" <?= $u['id_usuario'] == $idResponsable ? 'selected' : '' ?>>
      ...
      <div class="form-group full-width">
        ...
        <select id="usuarios_vinculados" name="usuarios_vinculados[]" multiple="multiple" style="width: 100%;">
          <?php foreach ($usuarios as $u): ?>
            <?php
              $isSelected = in_array($u['id_usuario'], $usuarios_vinculados_actuales);
              $isFixed = ($u['id_usuario'] == $idResponsable);
            ?>
            <option value="<?= htmlspecialchars($u['id_usuario']) ?>"
              <?= $isSelected ? 'selected' : '' ?>
              <?= $isFixed ? 'data-fixed="true"' : '' ?>>
              <?= htmlspecialchars($u['usuario']) ?><?= $isFixed ? ' (Responsable)' : '' ?>
            </option>
          <?php endforeach; ?>
        </select>
      </div>
...
<!-- Variable PHP para usar en JS -->
<script>
  const ID_RESPONSABLE = "<?= $idResponsable ?>";
</script>
...
<script>
  $(document).ready(function() {
    ...
    // Bloquear eliminación del usuario responsable
    $('#usuarios_vinculados').on('select2:unselecting', function (e) {
      if (e.params.args.data.id == ID_RESPONSABLE) {
        alert("No se puede quitar al responsable.");
        e.preventDefault();
      }
    });
  });
